...
/*
 * Client stubs for service: GatewayInterfaceStatistics
 * Functions that implement the generated GatewayInterfaceStatisticsClient interface
 */
...
type DefaultGatewayInterfaceStatisticsClient struct {
...
func NewDefaultGatewayInterfaceStatisticsClient(connector client.Connector) *DefaultGatewayInterfaceStatisticsClient {
	interfaceName := "com.vmware.nsx_policy.infra.tier_1s.segments.gateway_interface_statistics"
	...
	gIface := DefaultGatewayInterfaceStatisticsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	gIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	gIface.methodNameToDefMap["get"] = gIface.getMethodDefinition()
	return &gIface
}

func (gIface *DefaultGatewayInterfaceStatisticsClient) Get(tier1IdParam string, segmentIdParam string, cursorParam *string, edgePathParam *string, enforcementPointPathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.PolicyInterfaceStatistics, error) {
...
func (gIface *DefaultGatewayInterfaceStatisticsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGatewayInterfaceStatisticsClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(gatewayInterfaceStatisticsGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(gatewayInterfaceStatisticsGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceStatisticsClient.get method's errors.NotFound error - %s",
