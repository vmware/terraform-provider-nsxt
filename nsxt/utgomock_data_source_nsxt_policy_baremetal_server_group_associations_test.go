...
// data_source_nsxt_policy_baremetal_server_group_associations.go builds its
// BareMetalServerGroupAssociationsClient via a direct constructor call
// (api/infra.NewBareMetalServerGroupAssociationsClient), not a swappable
...
func bmsGroupAssociationAPIResponse(targetID, targetName, targetType string, isValid bool) nsxModel.PolicyResourceReferenceForEP {
	return nsxModel.PolicyResourceReferenceForEP{
		TargetId:          &targetID,
...
func TestMockDataSourceNsxtPolicyBareMetalServerGroupAssociationsRead(t *testing.T) {
	util.NsxVersion = "9.0.0"
	defer func() { util.NsxVersion = "" }()

	externalID := "bms-ext-1"

	t.Run("success with results", func(t *testing.T) {
		m, closeServer := newVapiRestTestServer(t, func(w http.ResponseWriter, r *http.Request) {
			require.Equal(t, "/policy/api/v1/infra/bms-group-associations", r.URL.Path)
		...
		ds := dataSourceNsxtPolicyBareMetalServerGroupAssociations()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{
			"external_id": externalID,
		})

		err := dataSourceNsxtPolicyBareMetalServerGroupAssociationsRead(d, m)
	...
	t.Run("missing external_id errors before any API call", func(t *testing.T) {
		...
		err := dataSourceNsxtPolicyBareMetalServerGroupAssociationsRead(d, newGoMockProviderClient())
		require.Error(t, err)
