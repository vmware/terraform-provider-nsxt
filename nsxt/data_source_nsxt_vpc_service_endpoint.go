...
func dataSourceNsxtVpcServiceEndpointRead(d *schema.ResourceData, m interface{}) error {
	...
	objInt, err := policyDataSourceResourceReadWithValidation(d, connector, getSessionContext(d, m), "VpcServiceEndpoint", query, false)
	if err != nil {
		return err
	}

	converter := bindings.NewTypeConverter()
	dataValue, errors := converter.ConvertToGolang(objInt, model.VpcServiceEndpointBindingType())
	if len(errors) > 0 {
		return fmt.Errorf("Failed to convert type for VpcServiceEndpoint: %v", errors[0])
	}
	obj := dataValue.(model.VpcServiceEndpoint)
	d.Set("service_endpoint_ip", obj.ServiceEndpointIp)
