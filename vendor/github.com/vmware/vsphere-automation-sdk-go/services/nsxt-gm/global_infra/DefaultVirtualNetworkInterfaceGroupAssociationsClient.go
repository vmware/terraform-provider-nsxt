...
/*
 * Client stubs for service: VirtualNetworkInterfaceGroupAssociations
 * Functions that implement the generated VirtualNetworkInterfaceGroupAssociationsClient interface
 */
...
type DefaultVirtualNetworkInterfaceGroupAssociationsClient struct {
...
func NewDefaultVirtualNetworkInterfaceGroupAssociationsClient(connector client.Connector) *DefaultVirtualNetworkInterfaceGroupAssociationsClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.virtual_network_interface_group_associations"
	...
	vIface := DefaultVirtualNetworkInterfaceGroupAssociationsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (vIface *DefaultVirtualNetworkInterfaceGroupAssociationsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := vIface.connector.GetApiProvider().Invoke(vIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (vIface *DefaultVirtualNetworkInterfaceGroupAssociationsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(vIface.interfaceName)
	typeConverter := vIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(virtualNetworkInterfaceGroupAssociationsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(virtualNetworkInterfaceGroupAssociationsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualNetworkInterfaceGroupAssociationsClient.list method's errors.NotFound error - %s",
