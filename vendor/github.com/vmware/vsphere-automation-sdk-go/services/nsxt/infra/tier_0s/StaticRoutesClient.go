...
type StaticRoutesClient interface {
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(tier0IdParam string, routeIdParam string, staticRoutesParam nsx_policyModel.StaticRoutes) error
...
func NewStaticRoutesClient(connector vapiProtocolClient_.Connector) *staticRoutesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_0s.static_routes")
...
func (sIface *staticRoutesClient) Patch(tier0IdParam string, routeIdParam string, staticRoutesParam nsx_policyModel.StaticRoutes) error {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
