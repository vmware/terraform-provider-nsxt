...
func NewUiViewsClient(connector vapiProtocolClient_.Connector) *uiViewsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.ui_views")
...
func (uIface *uiViewsClient) Create(viewParam nsx_policyModel.View) (nsx_policyModel.View, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.View
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.View), nil
	} else {
...
func (uIface *uiViewsClient) Get(tagParam *string, viewIdsParam *string, widgetIdParam *string) (nsx_policyModel.ViewList, error) {
	typeConverter := uIface.connector.TypeConverter()
	executionContext := uIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_policyModel.ViewList
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}
	...
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.ViewList), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), uIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
