...
func resourceNsxtPolicyIntrusionServicePolicyRule() *schema.Resource {
	ruleSchema := getSecurityPolicyAndGatewayRuleSchema(false, true, false, true)

	// Override action validation for EXEMPT version checking
	if actionSchema, exists := ruleSchema["action"]; exists {
		actionSchema.ValidateFunc = validateIdpsDfwRuleAction
	}
...
func intrusionServicePolicyRuleModelToSchema(d *schema.ResourceData, rule model.IdsRule) {
	...
	if rule.Oversubscription != nil {
		d.Set("oversubscription", *rule.Oversubscription)
	}
...
func resourceNsxtPolicyIntrusionServicePolicyRuleCreate(d *schema.ResourceData, m interface{}) error {
	...
	// Validate EXEMPT action version requirement (following codebase pattern)
	if action := d.Get("action").(string); action == "EXEMPT" {
		if !util.NsxVersionHigherOrEqual("9.1.0") {
			return fmt.Errorf("EXEMPT action requires NSX version 9.1.0 or higher")
		}
	}
