...
/*
 * Client stubs for service: GatewayInterfaceDadState
 * Functions that implement the generated GatewayInterfaceDadStateClient interface
 */
...
type DefaultGatewayInterfaceDadStateClient struct {
...
func NewDefaultGatewayInterfaceDadStateClient(connector client.Connector) *DefaultGatewayInterfaceDadStateClient {
	interfaceName := "com.vmware.nsx_policy.infra.tier_1s.segments.gateway_interface_dad_state"
	...
	gIface := DefaultGatewayInterfaceDadStateClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	gIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	gIface.methodNameToDefMap["get"] = gIface.getMethodDefinition()
	return &gIface
}

func (gIface *DefaultGatewayInterfaceDadStateClient) Get(tier1IdParam string, segmentIdParam string, enforcementPointPathParam *string, sourceParam *string) (model.InterfaceDADState, error) {
...
func (gIface *DefaultGatewayInterfaceDadStateClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGatewayInterfaceDadStateClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(gatewayInterfaceDadStateGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(gatewayInterfaceDadStateGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceDadStateClient.get method's errors.NotFound error - %s",
