...
/*
 * Client stubs for service: GatewayInterfaceArpTable
 * Functions that implement the generated GatewayInterfaceArpTableClient interface
 */
...
type DefaultGatewayInterfaceArpTableClient struct {
...
func NewDefaultGatewayInterfaceArpTableClient(connector client.Connector) *DefaultGatewayInterfaceArpTableClient {
	interfaceName := "com.vmware.nsx_policy.infra.tier_1s.segments.gateway_interface_arp_table"
	...
	gIface := DefaultGatewayInterfaceArpTableClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (gIface *DefaultGatewayInterfaceArpTableClient) List(tier1IdParam string, segmentIdParam string, cursorParam *string, edgePathParam *string, enforcementPointPathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.InterfaceArpTable, error) {
...
func (gIface *DefaultGatewayInterfaceArpTableClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGatewayInterfaceArpTableClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(gatewayInterfaceArpTableListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(gatewayInterfaceArpTableListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayInterfaceArpTableClient.list method's errors.NotFound error - %s",
