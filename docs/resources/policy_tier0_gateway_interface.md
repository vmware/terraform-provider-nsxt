...
* `segment_path` - (Optional) Policy path for the segment connected to this Tier-0 interface. Required for interfaces of type `SERVICE` and `EXTERNAL` unless `subnet_path` is set instead. Mutually exclusive with `subnet_path`.
* `subnet_path` - (Optional) Policy path of an overlay VPC subnet for centralized network attachment (alternative to `segment_path`). Mutually exclusive with `segment_path`. **Supported with NSX 9.2.0 or greater.**
