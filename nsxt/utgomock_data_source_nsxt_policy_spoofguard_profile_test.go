...
func TestUnitNsxt_dataSourceNsxtPolicySpoofGuardProfileRead(t *testing.T) {
	dsID := "spoofguard-profile-1"

	t.Run("success", func(t *testing.T) {
		sv := policyResourceToStructValue(t, gmModel.PolicyResource{
			Id: str(dsID), DisplayName: str("name"), Path: str("/infra/x"), ResourceType: str("SpoofGuardProfile"),
		})
		...
		ds := dataSourceNsxtPolicySpoofGuardProfile()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{"id": dsID})

		err := dataSourceNsxtPolicySpoofGuardProfileRead(d, newGoMockProviderClient())
