...
/*
 * Client stubs for service: GroupServiceAssociations
 * Functions that implement the generated GroupServiceAssociationsClient interface
 */
...
type DefaultGroupServiceAssociationsClient struct {
...
func NewDefaultGroupServiceAssociationsClient(connector client.Connector) *DefaultGroupServiceAssociationsClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.group_service_associations"
	...
	gIface := DefaultGroupServiceAssociationsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (gIface *DefaultGroupServiceAssociationsClient) List(intentPathParam string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.PolicyResourceReferenceListResult, error) {
	typeConverter := gIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(gIface.interfaceIdentifier, "list")
	sv := bindings.NewStructValueBuilder(groupServiceAssociationsListInputType(), typeConverter)
...
func (gIface *DefaultGroupServiceAssociationsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGroupServiceAssociationsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(groupServiceAssociationsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(groupServiceAssociationsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupServiceAssociationsClient.list method's errors.NotFound error - %s",
