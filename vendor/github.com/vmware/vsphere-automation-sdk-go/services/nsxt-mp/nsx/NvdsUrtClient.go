...
func NewNvdsUrtClient(connector vapiProtocolClient_.Connector) *nvdsUrtClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.nvds_urt")
...
func (nIface *nvdsUrtClient) Cleanup() error {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx.nvds_urt", "cleanup", inputDataValue, executionContext)
...
func (nIface *nvdsUrtClient) Ignoremigratestatus() error {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx.nvds_urt", "ignoremigratestatus", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), nIface.GetErrorBindingType(methodResult.Error().Name()))
