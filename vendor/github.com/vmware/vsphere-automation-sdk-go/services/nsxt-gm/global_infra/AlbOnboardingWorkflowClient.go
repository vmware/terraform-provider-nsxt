...
type AlbOnboardingWorkflowClient interface {
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(aLBControllerConfigurationParam nsx_global_policyModel.ALBControllerConfiguration) (nsx_global_policyModel.EnforcementPoint, error)
}
...
func NewAlbOnboardingWorkflowClient(connector vapiProtocolClient_.Connector) *albOnboardingWorkflowClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.alb_onboarding_workflow")
...
func (aIface *albOnboardingWorkflowClient) Delete(managedByParam string) error {
	...
	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.alb_onboarding_workflow", "delete", inputDataValue, executionContext)
...
func (aIface *albOnboardingWorkflowClient) Update(aLBControllerConfigurationParam nsx_global_policyModel.ALBControllerConfiguration) (nsx_global_policyModel.EnforcementPoint, error) {
	typeConverter := aIface.connector.TypeConverter()
	executionContext := aIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.EnforcementPoint
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.alb_onboarding_workflow", "update", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.EnforcementPoint
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.EnforcementPoint), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), aIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
