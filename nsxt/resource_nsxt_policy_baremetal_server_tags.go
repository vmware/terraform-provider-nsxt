...
func resourceNsxtPolicyBareMetalServerTagsImporter(d *schema.ResourceData, m interface{}) ([]*schema.ResourceData, error) {
	...
	if !isValidResourceID(importID) {
		return nil, fmt.Errorf("invalid import ID %q: expected bare metal server external_id", importID)
	}
...
func resourceNsxtPolicyBareMetalServerTagsRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		d.SetId("")
		log.Printf("[INFO] bare metal server %s not found, removing from state: %v", externalID, err)
		return nil
	}
...
func resourceNsxtPolicyBareMetalServerTagsDelete(d *schema.ResourceData, m interface{}) error {
	...
	if _, err := findBareMetalServerByExternalID(connector, ctx, externalID); err != nil {
		// If server doesn't exist, consider tags already removed
		d.SetId("")
		log.Printf("[INFO] bare metal server %s not found, considering tags already removed", externalID)
		return nil
	}
