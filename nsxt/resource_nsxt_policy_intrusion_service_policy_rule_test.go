...
func TestAccResourceNsxtPolicyIntrusionServicePolicyRule_exemptAction(t *testing.T) {
	testAccResourceNsxtPolicyIntrusionServicePolicyRuleExemptAction(t, false, func() {
		testAccPreCheck(t)
		testAccOnlyLocalManager(t)
...
func TestAccResourceNsxtPolicyIntrusionServicePolicyRule_exemptAction_multitenancy(t *testing.T) {
	testAccResourceNsxtPolicyIntrusionServicePolicyRuleExemptAction(t, true, func() {
		testAccPreCheck(t)
...
func testAccResourceNsxtPolicyIntrusionServicePolicyRuleExemptAction(t *testing.T, withContext bool, preCheck func()) {
	name := getAccTestResourceName()
	testResourceName := "nsxt_policy_intrusion_service_policy_rule.rule1"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNsxtPolicyIntrusionServicePolicyRuleExemptTemplate(name, withContext),
				Check: resource.ComposeTestCheckFunc(
					testAccNsxtPolicyIntrusionServicePolicyRuleExists(testResourceName),
					resource.TestCheckResourceAttr(testResourceName, "display_name", name),
...
func testAccNsxtPolicyIntrusionServicePolicyRuleExemptTemplate(name string, withContext bool) string {
	context := ""
	profile := ""
	profilePath := fmt.Sprintf("\"%s\"", policyDefaultIdsProfilePath)
	if withContext {
	...
	return profile + testAccNsxtPolicyIntrusionServicePolicyRuleParentDeps(withContext) + fmt.Sprintf(`
resource "nsxt_policy_intrusion_service_policy_rule" "rule1" {
%s
  display_name    = "%s"
  policy_path     = nsxt_policy_parent_intrusion_service_policy.parent.path
