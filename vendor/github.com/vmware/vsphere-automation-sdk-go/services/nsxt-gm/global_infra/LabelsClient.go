...
func NewLabelsClient(connector client.Connector) *labelsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.labels")
...
func (lIface *labelsClient) Delete(labelIdParam string) error {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.labels", "delete", inputDataValue, executionContext)
...
func (lIface *labelsClient) Patch(labelIdParam string, policyLabelParam model.PolicyLabel) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	lIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.labels", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), lIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (lIface *labelsClient) Update(labelIdParam string, policyLabelParam model.PolicyLabel) (model.PolicyLabel, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	lIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.labels", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicyLabel
	if methodResult.IsSuccess() {
