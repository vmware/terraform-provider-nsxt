...
/*
 * Client stubs for service: SiteCleanupPending
 * Functions that implement the generated SiteCleanupPendingClient interface
 */
...
type DefaultSiteCleanupPendingClient struct {
...
func NewDefaultSiteCleanupPendingClient(connector client.Connector) *DefaultSiteCleanupPendingClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.site_cleanup_pending"
	...
	sIface := DefaultSiteCleanupPendingClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	sIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	sIface.methodNameToDefMap["list"] = sIface.listMethodDefinition()
	return &sIface
}

func (sIface *DefaultSiteCleanupPendingClient) List(cursorParam *string, includedFieldsParam *string, intentPathParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.SiteCleanupPendingListResult, error) {
...
func (sIface *DefaultSiteCleanupPendingClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := sIface.connector.GetApiProvider().Invoke(sIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (sIface *DefaultSiteCleanupPendingClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(sIface.interfaceName)
	typeConverter := sIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(siteCleanupPendingListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(siteCleanupPendingListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultSiteCleanupPendingClient.list method's errors.NotFound error - %s",
