...
func resourceNsxtPolicyBareMetalServerInterfaceTagsRead(d *schema.ResourceData, m interface{}) error {
	...
	if err != nil {
		d.SetId("")
		log.Printf("[INFO] bare metal server interface %s not found, removing from state: %v", externalID, err)
		return nil
	}
...
func resourceNsxtPolicyBareMetalServerInterfaceTagsDelete(d *schema.ResourceData, m interface{}) error {
	...
	if _, err := findBareMetalServerInterfaceByExternalID(connector, ctx, externalID); err != nil {
		// If interface doesn't exist, consider tags already removed
		d.SetId("")
		log.Printf("[INFO] bare metal server interface %s not found, considering tags already removed", externalID)
		return nil
	}
