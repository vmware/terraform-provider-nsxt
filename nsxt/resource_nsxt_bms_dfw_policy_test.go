...
// TestAccResourceNsxtPolicyBMSSecurityPolicy_standalone tests DFW standalone policies with BMS groups
func TestAccResourceNsxtPolicyBMSSecurityPolicy_standalone(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// Verify standalone DFW policy creation with BMS groups
					testAccBMSDFWPolicyEnhancedCheck("nsxt_policy_security_policy.bms_standalone", testName+"-bms-standalone-policy"),
				),
			},
		},
	})
...
// TestAccResourceNsxtPolicyBMSSecurityPolicy_parent tests DFW parent policies with BMS groups
func TestAccResourceNsxtPolicyBMSSecurityPolicy_parent(t *testing.T) {
	testName := getAccTestResourceName()
	serverID := getTestBMSServerID()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.nsxt_policy_baremetal_servers.all", "results.#"),
					// Verify parent DFW policy creation with BMS groups
					testAccBMSDFWPolicyEnhancedCheck("nsxt_policy_security_policy.bms_parent", testName+"-bms-parent-policy"),
				),
			},
		},
	})
...
// TestAccResourceNsxtPolicyBMSSecurityPolicy_rules tests DFW standalone rules with BMS groups
func TestAccResourceNsxtPolicyBMSSecurityPolicy_rules(t *testing.T) {
	testName := getAccTestResourceName()
	serverID := getTestBMSServerID()
	interfaceID := getTestBMSInterfaceID()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.nsxt_policy_baremetal_servers.all", "results.#"),
					// Verify BMS group creation
					resource.TestCheckResourceAttrSet("nsxt_policy_group.bms_rule_source", "id"),
					resource.TestCheckResourceAttr("nsxt_policy_group.bms_rule_source", "display_name", testName+"-bms-rule-source"),
					resource.TestCheckResourceAttr("nsxt_policy_group.bms_rule_source", "group_type", "BareMetalServer"),
				),
			},
		},
	})
...
// TestAccResourceNsxtPolicyBMSSecurityPolicy_categories tests DFW policies covering all categories
func TestAccResourceNsxtPolicyBMSSecurityPolicy_categories(t *testing.T) {
	testName := getAccTestResourceName()
	serverID := getTestBMSServerID()
...
// TestAccResourceNsxtPolicyBMSSecurityPolicy_dataSources tests reading DFW policies with BMS groups via data sources
func TestAccResourceNsxtPolicyBMSSecurityPolicy_dataSources(t *testing.T) {
	testName := getAccTestResourceName()
	serverID := getTestBMSServerID()
...
// TestAccDataSourceNsxtPolicyBMSSecurityPolicyRule_dataSource tests reading DFW policy rules via data sources
func TestAccDataSourceNsxtPolicyBMSSecurityPolicyRule_dataSource(t *testing.T) {
	testName := getAccTestResourceName()
	serverID := getTestBMSServerID()
...
// TestAccResourceNsxtPolicyBMSSecurityPolicy_allScenarios tests comprehensive DFW scenarios with BMS
func TestAccResourceNsxtPolicyBMSSecurityPolicy_allScenarios(t *testing.T) {
	testName := getAccTestResourceName()
	serverID := getTestBMSServerID()
	interfaceID := getTestBMSInterfaceID()
...
// Enhanced validation function for DFW policy tests
func testAccBMSDFWPolicyEnhancedCheck(resourceName, expectedDisplayName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if _, ok := s.RootModule().Resources[resourceName]; ok {
			return resource.ComposeTestCheckFunc(
				...
				resource.TestCheckResourceAttrSet(resourceName, "rule.#"),
				// Verify at least one rule exists
				resource.TestCheckResourceAttrSet(resourceName, "rule.0.display_name"),
				resource.TestCheckResourceAttrSet(resourceName, "rule.0.action"),
			)(s)
		}
		return nil
	}
