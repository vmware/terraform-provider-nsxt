...
/*
 * Client stubs for service: UpgradeSummary
 * Functions that implement the generated UpgradeSummaryClient interface
 */
...
type DefaultUpgradeSummaryClient struct {
...
func NewDefaultUpgradeSummaryClient(connector client.Connector) *DefaultUpgradeSummaryClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.upgrade_summary"
	...
	uIface := DefaultUpgradeSummaryClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (uIface *DefaultUpgradeSummaryClient) List(currentVersionParam *string, cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.FederationUpgradeSummaryListResult, error) {
...
func (uIface *DefaultUpgradeSummaryClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := uIface.connector.GetApiProvider().Invoke(uIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (uIface *DefaultUpgradeSummaryClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(uIface.interfaceName)
	typeConverter := uIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(upgradeSummaryListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(upgradeSummaryListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUpgradeSummaryClient.list method's errors.NotFound error - %s",
