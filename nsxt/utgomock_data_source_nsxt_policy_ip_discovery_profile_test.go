...
func TestUnitNsxt_dataSourceNsxtPolicyIPDiscoveryProfileRead(t *testing.T) {
	dsID := "ip-discovery-profile-1"

	t.Run("success", func(t *testing.T) {
		sv := policyResourceToStructValue(t, gmModel.PolicyResource{
			Id: str(dsID), DisplayName: str("name"), Path: str("/infra/x"), ResourceType: str("IPDiscoveryProfile"),
		})
		...
		ds := dataSourceNsxtPolicyIPDiscoveryProfile()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{"id": dsID})

		err := dataSourceNsxtPolicyIPDiscoveryProfileRead(d, newGoMockProviderClient())
