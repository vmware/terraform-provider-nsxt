...
/*
 * Client stubs for service: GmOperationalState
 * Functions that implement the generated GmOperationalStateClient interface
 */
...
type DefaultGmOperationalStateClient struct {
...
func NewDefaultGmOperationalStateClient(connector client.Connector) *DefaultGmOperationalStateClient {
	interfaceName := "com.vmware.nsx_global_policy.gm_operational_state"
	...
	gIface := DefaultGmOperationalStateClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	gIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	gIface.methodNameToDefMap["get"] = gIface.getMethodDefinition()
	return &gIface
}

func (gIface *DefaultGmOperationalStateClient) Get() (model.GmOperationalState, error) {
...
func (gIface *DefaultGmOperationalStateClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGmOperationalStateClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(gmOperationalStateGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(gmOperationalStateGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGmOperationalStateClient.get method's errors.NotFound error - %s",
