...
type PartnerServicesClient interface {
	...
	Get(serviceNameParam string) (nsx_policyModel.ServiceDefinition, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.ServiceInsertionServiceListResult, error)
}
...
func NewPartnerServicesClient(connector vapiProtocolClient_.Connector) *partnerServicesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.partner_services")
...
func (pIface *partnerServicesClient) GetErrorBindingType(errorName string) vapiBindings_.BindingType {
...
func (pIface *partnerServicesClient) Get(serviceNameParam string) (nsx_policyModel.ServiceDefinition, error) {
	...
	sv := vapiBindings_.NewStructValueBuilder(partnerServicesGetInputType(), typeConverter)
...
func (pIface *partnerServicesClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.ServiceInsertionServiceListResult, error) {
	typeConverter := pIface.connector.TypeConverter()
	executionContext := pIface.connector.NewExecutionContext()
	...
	sv := vapiBindings_.NewStructValueBuilder(partnerServicesListInputType(), typeConverter)
