...
// TestAccResourceNsxtPolicyDnsService_forwarderConfigPartial sets only
// upstream_servers in forwarder_config and relies on withIdempotencyChecks'
// PlanOnly step to catch a permadiff if cache_size isn't Optional+Computed.
func TestAccResourceNsxtPolicyDnsService_forwarderConfigPartial(t *testing.T) {
	testResourceName := "nsxt_policy_dns_service.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Providers: testAccProviders,
		CheckDestroy: func(state *terraform.State) error {
			return testAccNsxtPolicyDnsServiceCheckDestroy(state, displayName)
		},
		Steps: withIdempotencyChecks([]resource.TestStep{
			{
				Config: testAccNsxtPolicyDnsServiceForwarderConfigPartialTemplate(displayName),
				Check: resource.ComposeTestCheckFunc(
					testAccNsxtPolicyDnsServiceExists(testResourceName),
					resource.TestCheckResourceAttr(testResourceName, "forwarder_config.#", "1"),
					resource.TestCheckResourceAttr(testResourceName, "forwarder_config.0.upstream_servers.0", "8.8.8.8"),
				),
			},
		}),
	})
...
func testAccNsxtPolicyDnsServiceForwarderConfigPartialTemplate(displayName string) string {
