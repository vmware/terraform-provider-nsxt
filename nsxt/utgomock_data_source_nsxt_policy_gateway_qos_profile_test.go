...
func TestUnitNsxt_dataSourceNsxtPolicyGatewayQosProfileRead(t *testing.T) {
	dsID := "gw-qos-profile-1"

	t.Run("success", func(t *testing.T) {
		sv := policyResourceToStructValue(t, gmModel.PolicyResource{
			Id: str(dsID), DisplayName: str("name"), Path: str("/infra/x"), ResourceType: str("GatewayQosProfile"),
		})
		...
		ds := dataSourceNsxtPolicyGatewayQosProfile()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{"id": dsID})

		err := dataSourceNsxtPolicyGatewayQosProfileRead(d, newGoMockProviderClient())
