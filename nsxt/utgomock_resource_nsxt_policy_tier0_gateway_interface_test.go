...
func TestUnitNsxt_policyTier0GatewayInterfaceOspfSet(t *testing.T) {
	...
	t.Run("fails on non-external interface", func(t *testing.T) {
		d := schema.TestResourceDataRaw(t, res.Schema, map[string]interface{}{
			"type": nsxModel.Tier0Interface_TYPE_SERVICE,
			"ospf": []interface{}{
				map[string]interface{}{"enabled": true},
			},
		})
	...
	t.Run("populates the Ospf struct on an external interface", func(t *testing.T) {
		d := schema.TestResourceDataRaw(t, res.Schema, map[string]interface{}{
			...
			"ospf": []interface{}{
				map[string]interface{}{
					...
					"network_type":     nsxModel.PolicyInterfaceOspfConfig_NETWORK_TYPE_P2P,
					"hello_interval":   5,
					"dead_interval":    20,
				},
			},
		...
		assert.Equal(t, nsxModel.PolicyInterfaceOspfConfig_NETWORK_TYPE_P2P, *obj.Ospf.NetworkType)
