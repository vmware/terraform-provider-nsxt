...
func NewDefaultBatchClient(connector client.Connector) *DefaultBatchClient {
	interfaceName := "com.vmware.nsx_policy.batch"
	...
	bIface.methodNameToDefMap["create"] = bIface.createMethodDefinition()
	return &bIface
}

func (bIface *DefaultBatchClient) Create(batchRequestParam model.BatchRequest, atomicParam *bool) (model.BatchResponse, error) {
	typeConverter := bIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(bIface.interfaceIdentifier, "create")
...
func (bIface *DefaultBatchClient) createMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(bIface.interfaceName)
	typeConverter := bIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(batchCreateInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(batchCreateOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBatchClient.create method's errors.NotFound error - %s",
