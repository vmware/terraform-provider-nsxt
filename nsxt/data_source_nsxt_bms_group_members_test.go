		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// Verify group members structure when available
					testAccBMSGroupMembersEnhancedCheck(testResourceName),
				),
			},
		},
	})
...
resource "nsxt_policy_group" "bms_members_basic" {
...
data "nsxt_policy_group_baremetal_server_members" "test" {
  domain   = "default"
  group_id = nsxt_policy_group.bms_members_basic.id
}
...
func testAccNsxtPolicyGroupBareMetalServerMembersWithEnforcementPointTemplate(testName string) string {
	return fmt.Sprintf(`
# Create a BMS group with a server member
resource "nsxt_policy_group" "bms_members_enforcement" {
  display_name = "%s-bms-group"
...
data "nsxt_policy_group_baremetal_server_members" "test" {
  domain                 = "default"
  group_id              = nsxt_policy_group.bms_members_enforcement.id
...
resource "nsxt_policy_group" "bms_members_empty" {
...
data "nsxt_policy_group_baremetal_server_members" "test" {
  domain   = "default"
  group_id = nsxt_policy_group.bms_members_empty.id
}
`, testName)
}
...
resource "nsxt_policy_group" "bms_members_multiple" {
...
data "nsxt_policy_group_baremetal_server_members" "test" {
  domain   = "default"
  group_id = nsxt_policy_group.bms_members_multiple.id
}
`, testName, getTestBMSServerID())
}
...
resource "nsxt_policy_group" "bmsi_members_basic" {
...
data "nsxt_policy_group_baremetal_server_interface_members" "test" {
  domain   = "default"
  group_id = nsxt_policy_group.bmsi_members_basic.id
}
...
func testAccNsxtPolicyGroupBareMetalServerInterfaceMembersWithEnforcementPointTemplate(testName string) string {
	return fmt.Sprintf(`
# Create a BMS group with an interface member
resource "nsxt_policy_group" "bmsi_members_enforcement" {
  display_name = "%s-bmsi-group"
...
data "nsxt_policy_group_baremetal_server_interface_members" "test" {
  domain                 = "default"
  group_id              = nsxt_policy_group.bmsi_members_enforcement.id
  enforcement_point_path = "/infra/sites/default/enforcement-points/default"
}
`, testName, getTestBMSInterfaceID())
}
...
resource "nsxt_policy_group" "bmsi_members_empty" {
...
data "nsxt_policy_group_baremetal_server_interface_members" "test" {
  domain   = "default"
  group_id = nsxt_policy_group.bmsi_members_empty.id
}
`, testName)
}
...
resource "nsxt_policy_group" "bmsi_members_multiple" {
...
data "nsxt_policy_group_baremetal_server_interface_members" "test" {
  domain   = "default"
  group_id = nsxt_policy_group.bmsi_members_multiple.id
}
`, testName, getTestBMSInterfaceID())
}

// Enhanced validation function for group members tests
func testAccBMSGroupMembersEnhancedCheck(dataSourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if rs, ok := s.RootModule().Resources[dataSourceName]; ok {
			if rs.Primary.Attributes["items.#"] != "0" {
				// If we have member items, Validate fields that exist in group members
				return resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet(dataSourceName, "items.0.external_id"),
