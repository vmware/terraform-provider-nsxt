...
type GlobalInfraClient interface {
	...
	Get(basePathParam *string, filterParam *string, typeFilterParam *string) (nsx_global_policyModel.Infra, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(infraParam nsx_global_policyModel.Infra, enforceRevisionCheckParam *bool) error
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(infraParam nsx_global_policyModel.Infra) (nsx_global_policyModel.Infra, error)
}
...
func NewGlobalInfraClient(connector vapiProtocolClient_.Connector) *globalInfraClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra")
...
func (gIface *globalInfraClient) Get(basePathParam *string, filterParam *string, typeFilterParam *string) (nsx_global_policyModel.Infra, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.Infra
...
func (gIface *globalInfraClient) Patch(infraParam nsx_global_policyModel.Infra, enforceRevisionCheckParam *bool) error {
	typeConverter := gIface.connector.TypeConverter()
	executionContext := gIface.connector.NewExecutionContext()
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), gIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (gIface *globalInfraClient) Update(infraParam nsx_global_policyModel.Infra) (nsx_global_policyModel.Infra, error) {
	typeConverter := gIface.connector.TypeConverter()
	executionContext := gIface.connector.NewExecutionContext()
	...
	sv := vapiBindings_.NewStructValueBuilder(globalInfraUpdateInputType(), typeConverter)
	sv.AddStructField("Infra", infraParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.Infra
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra", "update", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.Infra
	if methodResult.IsSuccess() {
