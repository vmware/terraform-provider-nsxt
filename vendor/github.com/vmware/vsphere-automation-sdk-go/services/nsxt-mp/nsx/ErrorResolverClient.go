...
type ErrorResolverClient interface {
	...
	Get(errorIdParam string) (nsxModel.ErrorResolverInfo, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List() (nsxModel.ErrorResolverInfoList, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Resolveerror(errorResolverMetadataListParam nsxModel.ErrorResolverMetadataList) error
}
...
func NewErrorResolverClient(connector vapiProtocolClient_.Connector) *errorResolverClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.error_resolver")
...
func (eIface *errorResolverClient) Get(errorIdParam string) (nsxModel.ErrorResolverInfo, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ErrorResolverInfo
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx.error_resolver", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.ErrorResolverInfo
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ErrorResolverInfo), nil
	} else {
...
func (eIface *errorResolverClient) List() (nsxModel.ErrorResolverInfoList, error) {
	typeConverter := eIface.connector.TypeConverter()
	executionContext := eIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.ErrorResolverInfoList
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx.error_resolver", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.ErrorResolverInfoList
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ErrorResolverInfoList), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), eIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
...
func (eIface *errorResolverClient) Resolveerror(errorResolverMetadataListParam nsxModel.ErrorResolverMetadataList) error {
	typeConverter := eIface.connector.TypeConverter()
	executionContext := eIface.connector.NewExecutionContext()
	...
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx.error_resolver", "resolveerror", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), eIface.GetErrorBindingType(methodResult.Error().Name()))
