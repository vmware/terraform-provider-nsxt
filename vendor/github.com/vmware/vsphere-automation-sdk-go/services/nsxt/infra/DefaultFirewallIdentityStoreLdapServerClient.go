...
/*
 * Client stubs for service: FirewallIdentityStoreLdapServer
 * Functions that implement the generated FirewallIdentityStoreLdapServerClient interface
 */
...
type DefaultFirewallIdentityStoreLdapServerClient struct {
...
func NewDefaultFirewallIdentityStoreLdapServerClient(connector client.Connector) *DefaultFirewallIdentityStoreLdapServerClient {
	interfaceName := "com.vmware.nsx_policy.infra.firewall_identity_store_ldap_server"
	...
	fIface := DefaultFirewallIdentityStoreLdapServerClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (fIface *DefaultFirewallIdentityStoreLdapServerClient) Create(directoryLdapServerParam model.DirectoryLdapServer, actionParam string, enforcementPointPathParam *string) (model.DirectoryLdapServerStatus, error) {
...
func (fIface *DefaultFirewallIdentityStoreLdapServerClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := fIface.connector.GetApiProvider().Invoke(fIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (fIface *DefaultFirewallIdentityStoreLdapServerClient) createMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(fIface.interfaceName)
	typeConverter := fIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(firewallIdentityStoreLdapServerCreateInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(firewallIdentityStoreLdapServerCreateOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreLdapServerClient.create method's errors.NotFound error - %s",
