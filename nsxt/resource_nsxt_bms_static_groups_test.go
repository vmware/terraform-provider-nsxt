...
`, testName, os.Getenv("NSXT_TEST_BMS_SERVER"))
}
...
`, testName, os.Getenv("NSXT_TEST_BMS_INTERFACE"))
}
...
resource "nsxt_policy_group" "bms_nested_group" {
  ...
  criteria {
    path_expression {
      member_paths = [nsxt_policy_group.bms_base_group[0].path]
    }
  }
}
...
data "nsxt_policy_group" "bms_nested_group_read" {
  count = local.has_servers ? 1 : 0
  id = nsxt_policy_group.bms_nested_group[0].id
}
...
output "nested_group" {
  value = {
    ...
    base_group_path = local.has_servers ? nsxt_policy_group.bms_base_group[0].path : ""
    nested_group_path = local.has_servers ? nsxt_policy_group.bms_nested_group[0].path : ""
  }
}
`, testName, testName)
}
...
data "nsxt_policy_group" "bms_server_group" {
  count = local.has_servers ? 1 : 0
  id    = nsxt_policy_group.bms_server_group[0].id
}

# Output data source information
output "group_data_source" {
  value = {
    has_group = local.has_servers
    group_path = local.has_servers ? data.nsxt_policy_group.bms_server_group[0].path : ""
    group_display_name = local.has_servers ? data.nsxt_policy_group.bms_server_group[0].display_name : ""
  }
}
`, testName)
}
