...
func TestUnitNsxt_dataSourceNsxtPolicyQosProfileRead(t *testing.T) {
	dsID := "qos-profile-1"

	t.Run("success", func(t *testing.T) {
		sv := policyResourceToStructValue(t, gmModel.PolicyResource{
			Id: str(dsID), DisplayName: str("name"), Path: str("/infra/x"), ResourceType: str("QoSProfile"),
		})
		...
		ds := dataSourceNsxtPolicyQosProfile()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{"id": dsID})

		err := dataSourceNsxtPolicyQosProfileRead(d, newGoMockProviderClient())
