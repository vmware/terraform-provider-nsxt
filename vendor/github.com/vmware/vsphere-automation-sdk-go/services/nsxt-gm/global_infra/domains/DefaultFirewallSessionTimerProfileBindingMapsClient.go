...
/*
 * Client stubs for service: FirewallSessionTimerProfileBindingMaps
 * Functions that implement the generated FirewallSessionTimerProfileBindingMapsClient interface
 */
...
type DefaultFirewallSessionTimerProfileBindingMapsClient struct {
...
func NewDefaultFirewallSessionTimerProfileBindingMapsClient(connector client.Connector) *DefaultFirewallSessionTimerProfileBindingMapsClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.domains.firewall_session_timer_profile_binding_maps"
	...
	fIface := DefaultFirewallSessionTimerProfileBindingMapsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (fIface *DefaultFirewallSessionTimerProfileBindingMapsClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.PolicyFirewallSessionTimerProfileBindingMapListResult, error) {
...
func (fIface *DefaultFirewallSessionTimerProfileBindingMapsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := fIface.connector.GetApiProvider().Invoke(fIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (fIface *DefaultFirewallSessionTimerProfileBindingMapsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(fIface.interfaceName)
	typeConverter := fIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(firewallSessionTimerProfileBindingMapsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(firewallSessionTimerProfileBindingMapsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallSessionTimerProfileBindingMapsClient.list method's errors.NotFound error - %s",
