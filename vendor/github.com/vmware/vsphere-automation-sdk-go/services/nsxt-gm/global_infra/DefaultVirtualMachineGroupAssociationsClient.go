...
/*
 * Client stubs for service: VirtualMachineGroupAssociations
 * Functions that implement the generated VirtualMachineGroupAssociationsClient interface
 */
...
type DefaultVirtualMachineGroupAssociationsClient struct {
...
func NewDefaultVirtualMachineGroupAssociationsClient(connector client.Connector) *DefaultVirtualMachineGroupAssociationsClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.virtual_machine_group_associations"
	...
	vIface := DefaultVirtualMachineGroupAssociationsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (vIface *DefaultVirtualMachineGroupAssociationsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := vIface.connector.GetApiProvider().Invoke(vIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (vIface *DefaultVirtualMachineGroupAssociationsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(vIface.interfaceName)
	typeConverter := vIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(virtualMachineGroupAssociationsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(virtualMachineGroupAssociationsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVirtualMachineGroupAssociationsClient.list method's errors.NotFound error - %s",
