...
type BepStatisticsClient interface {
	...
	// @return com.vmware.nsx_global_policy.model.BridgeEndpointStatistics
	//
	...
	Get(tier1IdParam string, segmentIdParam string, bridgeProfilePathParam string, enforcementPointPathParam *string) (nsx_global_policyModel.BridgeEndpointStatistics, error)
}
...
func NewBepStatisticsClient(connector vapiProtocolClient_.Connector) *bepStatisticsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_1s.segments.bep_statistics")
...
func (bIface *bepStatisticsClient) Get(tier1IdParam string, segmentIdParam string, bridgeProfilePathParam string, enforcementPointPathParam *string) (nsx_global_policyModel.BridgeEndpointStatistics, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.BridgeEndpointStatistics
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := bIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_1s.segments.bep_statistics", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.BridgeEndpointStatistics
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.BridgeEndpointStatistics), nil
	} else {
