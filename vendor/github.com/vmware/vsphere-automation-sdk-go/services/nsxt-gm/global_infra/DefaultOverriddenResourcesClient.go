...
/*
 * Client stubs for service: OverriddenResources
 * Functions that implement the generated OverriddenResourcesClient interface
 */
...
type DefaultOverriddenResourcesClient struct {
...
func NewDefaultOverriddenResourcesClient(connector client.Connector) *DefaultOverriddenResourcesClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.overridden_resources"
	...
	oIface := DefaultOverriddenResourcesClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (oIface *DefaultOverriddenResourcesClient) List(intentPathParam *string, sitePathParam *string) (model.OverriddenResourceListResult, error) {
...
func (oIface *DefaultOverriddenResourcesClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := oIface.connector.GetApiProvider().Invoke(oIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (oIface *DefaultOverriddenResourcesClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(oIface.interfaceName)
	typeConverter := oIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(overriddenResourcesListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(overriddenResourcesListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOverriddenResourcesClient.list method's errors.NotFound error - %s",
