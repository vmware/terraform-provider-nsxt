...
// TestAccResourceNsxtPolicyBMSGroup_staticServer tests static BMS server groups
func TestAccResourceNsxtPolicyBMSGroup_staticServer(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("nsxt_policy_group.bms_servers_static", "display_name", testName+"-bms-servers-static"),
					// Verify external ID expression criteria
					testAccBMSStaticGroupsEnhancedCheck("nsxt_policy_group.bms_servers_static", "BareMetalServer"),
				),
			},
		},
	})
...
// TestAccResourceNsxtPolicyBMSGroup_staticInterface tests static BMS interface groups
func TestAccResourceNsxtPolicyBMSGroup_staticInterface(t *testing.T) {
	testName := getAccTestResourceName()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("nsxt_policy_group.bms_interfaces_static", "display_name", testName+"-bms-interfaces-static"),
					// Verify external ID expression criteria for interfaces
					testAccBMSStaticGroupsEnhancedCheck("nsxt_policy_group.bms_interfaces_static", "BareMetalServerInterface"),
				),
			},
		},
	})
...
// TestAccResourceNsxtPolicyBMSGroup_staticNested tests nested BMS groups (group as member)
func TestAccResourceNsxtPolicyBMSGroup_staticNested(t *testing.T) {
	testName := getAccTestResourceName()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// Verify nested group creation and path expressions
					testAccBMSStaticGroupsNestedCheck(testName),
				),
			},
		},
	})
...
// TestAccResourceNsxtPolicyBMSGroup_staticDataSources tests reading static groups via data sources
func TestAccResourceNsxtPolicyBMSGroup_staticDataSources(t *testing.T) {
	testName := getAccTestResourceName()
...
// Enhanced validation functions for static groups
func testAccBMSStaticGroupsEnhancedCheck(resourceName, memberType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if _, ok := s.RootModule().Resources[resourceName]; ok {
			return resource.ComposeTestCheckFunc(
				resource.TestCheckResourceAttrSet(resourceName, "id"),
				resource.TestCheckResourceAttrSet(resourceName, "path"),
				resource.TestCheckResourceAttr(resourceName, "group_type", "BareMetalServer"),
				resource.TestCheckResourceAttrSet(resourceName, "criteria.#"),
				// Verify external_id_expression exists
				resource.TestCheckResourceAttrSet(resourceName, "criteria.0.external_id_expression.#"),
			)(s)
		}
		return nil
	}
...
func testAccBMSStaticGroupsNestedCheck(testName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Check individual groups first
		serverGroupName := "nsxt_policy_group.servers_static"
		if _, ok := s.RootModule().Resources[serverGroupName]; ok {
			checkFunc := resource.ComposeTestCheckFunc(
				resource.TestCheckResourceAttr(serverGroupName, "display_name", testName+"-servers-static"),
				resource.TestCheckResourceAttr(serverGroupName, "group_type", "BareMetalServer"),
			)
		...
		// Check nested group
		nestedGroupName := "nsxt_policy_group.nested_group"
		if _, ok := s.RootModule().Resources[nestedGroupName]; ok {
			return resource.ComposeTestCheckFunc(
				resource.TestCheckResourceAttr(nestedGroupName, "display_name", testName+"-nested-group"),
				resource.TestCheckResourceAttr(nestedGroupName, "group_type", "BareMetalServer"),
				// Verify path_expression exists for nested groups
				resource.TestCheckResourceAttrSet(nestedGroupName, "criteria.0.path_expression.#"),
			)(s)
		}
