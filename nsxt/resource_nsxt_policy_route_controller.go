...
func initRouteControllerBgpChild(bgpObj *model.RouteControllerBgpRoutingConfig, markDelete bool) (*data.StructValue, error) {
	...
	if markDelete {
		...
		child.RouteControllerBgpRoutingConfig = &model.RouteControllerBgpRoutingConfig{
			Id:           &bgpID,
			ResourceType: &bgpType,
		}
	} else {
		bgpType := "RouteControllerBgpRoutingConfig"
		bgpObj.Id = &bgpID
...
func resourceNsxtPolicyRouteControllerUpdate(d *schema.ResourceData, m interface{}) error {
	...
	// When bgp_config is removed, delete the BGP child directly via the BGP API.
	// Using a MarkedForDelete H-API tombstone is rejected by NSX because it still
	// validates required fields (e.g. local_as_num) on the child object.
	if d.HasChange("bgp_config") {
		bgpList := d.Get("bgp_config").([]interface{})
		if len(bgpList) == 0 || bgpList[0] == nil {
			bgpClient := cliRouteControllerBgpClient(context, connector)
			if bgpClient == nil {
				return policyResourceNotSupportedError()
			}
			if err := bgpClient.Delete(id); err != nil && !isNotFoundError(err) {
				return handleUpdateError("RouteController BGP config", id, err)
			}
		}
	}
