...
func NewDefaultGroupAssociationsClient(connector client.Connector) *DefaultGroupAssociationsClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.group_associations"
...
func (gIface *DefaultGroupAssociationsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(groupAssociationsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(groupAssociationsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupAssociationsClient.list method's errors.NotFound error - %s",
