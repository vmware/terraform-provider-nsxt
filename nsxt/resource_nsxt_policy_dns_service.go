...
// dnsServiceParentsFromPath parses the policy path of a resource nested
// under a DnsService (e.g. DnsZone, DnsRule) into its org/project/dns-service
// components.
func dnsServiceParentsFromPath(parentPath string) (org, project, dnsServiceID string, err error) {
	parents, pathErr := parseStandardPolicyPathVerifySize(parentPath, 3, policyDnsServicePathExample)
	if pathErr != nil {
		return "", "", "", pathErr
	}
