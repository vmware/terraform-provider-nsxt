...
import (
	...
	"github.com/vmware/vsphere-automation-sdk-go/services/nsxt-gm/global_infra/realized_state"
	"time"
)

func dataSourceNsxtPolicyGMRealizationInfo() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceNsxtPolicyGMRealizationInfoRead,

		Schema: map[string]*schema.Schema{
			"id": getDataSourceIDSchema(),
			"path": {
				Type:         schema.TypeString,
				Description:  "The path for the policy global manager resource",
				Required:     true,
				ValidateFunc: validatePolicyPath(),
			},
			"site_path": {
				Type:         schema.TypeString,
				Description:  "The path for the policy site",
				Required:     true,
				ValidateFunc: validatePolicyPath(),
			},
			"entity_type": {
...
func dataSourceNsxtPolicyGMRealizationInfoRead(d *schema.ResourceData, m interface{}) error {
	...
	stateConf := &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			...
			realizationResult, realizationError := client.List(path, &sitePath)
			state := "UNKNOWN"
