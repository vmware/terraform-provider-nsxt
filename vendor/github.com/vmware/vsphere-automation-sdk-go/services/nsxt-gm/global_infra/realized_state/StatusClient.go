...
type StatusClient interface {
	...
	// @return com.vmware.nsx_global_policy.model.ConsolidatedRealizedStatus
	//
	...
	Get(intentPathParam string, includeEnforcedStatusParam *bool, sitePathParam *string) (nsx_global_policyModel.ConsolidatedRealizedStatus, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.realized_state.status")
...
func (sIface *statusClient) Get(intentPathParam string, includeEnforcedStatusParam *bool, sitePathParam *string) (nsx_global_policyModel.ConsolidatedRealizedStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.ConsolidatedRealizedStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.realized_state.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.ConsolidatedRealizedStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.ConsolidatedRealizedStatus), nil
	} else {
