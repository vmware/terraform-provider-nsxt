...
/*
 * Client stubs for service: DadState
 * Functions that implement the generated DadStateClient interface
 */
...
type DefaultDadStateClient struct {
...
func NewDefaultDadStateClient(connector client.Connector) *DefaultDadStateClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.tier_1s.locale_services.interfaces.dad_state"
	...
	dIface := DefaultDadStateClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	dIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	dIface.methodNameToDefMap["get"] = dIface.getMethodDefinition()
	return &dIface
}

func (dIface *DefaultDadStateClient) Get(tier1IdParam string, localeServiceIdParam string, interfaceIdParam string, enforcementPointPathParam *string) (model.InterfaceDADState, error) {
...
func (dIface *DefaultDadStateClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := dIface.connector.GetApiProvider().Invoke(dIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (dIface *DefaultDadStateClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(dIface.interfaceName)
	typeConverter := dIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(dadStateGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(dadStateGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDadStateClient.get method's errors.NotFound error - %s",
