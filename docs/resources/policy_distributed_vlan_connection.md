...
* `subnet_extension_connection` - (Optional) Controls the connectivity mode for VPC Subnets referencing this distributed VLAN connection. This can be one of DISABLED/ENABLED_L2/ENABLED_L2_AND_L3.
* `associated_ip_block_paths` - (Optional) List of IP address block(s) that are associated with the distributed vlan connection.
