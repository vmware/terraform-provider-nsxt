...
type GatewayInterfaceDadStateClient interface {
	...
	Get(tier1IdParam string, segmentIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_global_policyModel.InterfaceDADState, error)
}
...
func NewGatewayInterfaceDadStateClient(connector vapiProtocolClient_.Connector) *gatewayInterfaceDadStateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.tier_1s.segments.gateway_interface_dad_state")
...
func (gIface *gatewayInterfaceDadStateClient) Get(tier1IdParam string, segmentIdParam string, enforcementPointPathParam *string, sourceParam *string) (nsx_global_policyModel.InterfaceDADState, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.InterfaceDADState
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.tier_1s.segments.gateway_interface_dad_state", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.InterfaceDADState
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.InterfaceDADState), nil
	} else {
