...
* `enabled` - (Optional) Enable proxy configuration. Default is `false`.
* `scheme` - (Optional) Proxy scheme. Valid values are `HTTP` and `HTTPS`. Default is `HTTP`.
...
* `password` - (Optional) Password for proxy authentication. This field is sensitive.
...
* **Singleton Resource**: Only one proxy configuration exists per NSX environment. Declare this resource only once in your Terraform configuration.
* **Display Name**: The display name is managed by NSX and cannot be customized.
* **Proxy Validation**: When `enabled` is set to `true`, NSX validates connectivity to the proxy server. The test will fail if the proxy is unreachable.
* **Delete Behavior**: Deleting this resource disables the proxy configuration in NSX rather than removing it. The configuration persists in NSX.
* **Password Security**: The password is write-only and not returned by the NSX API. It won't appear in state refresh operations.
* **HTTPS Proxies**: When using HTTPS scheme, you may need to provide a `certificate_id`. The certificate must be imported to NSX trust management first.
