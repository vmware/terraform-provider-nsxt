...
// validateDnsRuleActionConstraints enforces the cross-field rules that NSX
// applies to DnsRule: FORWARD rules take exactly one of upstream_servers or
// shared_zone_path, and domain_patterns is required unless action_type is
// FORWARD with shared_zone_path set (in which case patterns are derived from
// the shared zone).
func validateDnsRuleActionConstraints(obj model.DnsRule) error {
	...
	if len(obj.DomainPatterns) == 0 {
		return fmt.Errorf("domain_patterns is required unless action_type is FORWARD and shared_zone_path is set")
	}
	return nil
}
...
func resourceNsxtPolicyDnsRuleCreate(d *schema.ResourceData, m interface{}) error {
	...
	org, project, dnsServiceID, pathErr := dnsServiceParentsFromPath(parentPath)
	if pathErr != nil {
		return pathErr
	}

	obj := policyDnsRuleFromSchema(d)
	if err := validateDnsRuleActionConstraints(obj); err != nil {
		return err
	}
