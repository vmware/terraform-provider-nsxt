...
// TestAccBMSValidationDFWOnly tests that BMS is only supported in DFW (not GFW)
func TestAccBMSValidationDFWOnly(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccBMSValidationDFWOnlyTemplate(testName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.nsxt_policy_baremetal_servers.all", "results.#"),
					// This should work - BMS in DFW is supported
				),
			},
		},
	})
...
// TestAccBMSValidationLayer34Only tests that only Layer 3/4 rules are supported with BMS
func TestAccBMSValidationLayer34Only(t *testing.T) {
	testName := getAccTestResourceName()
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccBMSValidationLayer34OnlyTemplate(testName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.nsxt_policy_baremetal_servers.all", "results.#"),
					// Layer 3/4 rules should work with BMS
				),
			},
		},
	})
...
// TestAccBMSValidationInvalidExternalID tests error handling for invalid external IDs
func TestAccBMSValidationInvalidExternalID(t *testing.T) {
	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			testAccOnlyLocalManager(t)
			testAccPreCheck(t)
...
func testAccBMSValidationDFWOnlyTemplate(testName string) string {
	return fmt.Sprintf(`
...
`, testName, os.Getenv("NSXT_TEST_BMS_SERVER"), testName)
}

func testAccBMSValidationLayer34OnlyTemplate(testName string) string {
	return fmt.Sprintf(`
# Data source to verify BMS inventory is accessible
data "nsxt_policy_baremetal_servers" "all" {}
...
`, testName, os.Getenv("NSXT_TEST_BMS_SERVER"), testName, testName, testName)
}
