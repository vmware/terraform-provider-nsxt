...
* `advanced_configuration` - (Optional) Advanced configuration for virtual network appliances in the cluster. This block supports:
    * `core_allocation_profile` - (Optional) Core allocation profile for VNAs in the cluster. Defines core allocation for new or redeployed VNAs. Supported values: `L4FORWARDING`, `L7LBSERVICE`, `L4LBSERVICE`. For `VPC_SERVICES` clusters, `L4LBSERVICE` is the default when not set. A manual reboot is required for a profile change to take effect on existing appliances.
