...
func TestUnitNsxt_dataSourceNsxtPolicyDnsZoneRead(t *testing.T) {
	parentPath := "/orgs/default/projects/proj-1/dns-services/svc-1"
	rt := "DnsZone"
	sv := policyResourceToStructValue(t, gmModel.PolicyResource{
		Id: str("dns-zone-1"), DisplayName: str("zone-name"), Path: str(parentPath + "/dns-zones/dns-zone-1"), ResourceType: &rt,
	})
	...
	t.Run("by id", func(t *testing.T) {
		...
		ds := dataSourceNsxtPolicyDnsZone()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{
			"id":          "dns-zone-1",
			"parent_path": parentPath,
		})

		err := dataSourceNsxtPolicyDnsZoneRead(d, newGoMockProviderClient())
		require.NoError(t, err)
		assert.Equal(t, "dns-zone-1", d.Id())
	})
