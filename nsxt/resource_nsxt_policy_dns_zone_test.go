		...
		Steps: withIdempotencyChecks([]resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(testResourceName, "soa.0.retry_interval", "900"),
					resource.TestCheckResourceAttr(testResourceName, "soa.0.expire_time", "604800"),
					resource.TestCheckResourceAttr(testResourceName, "soa.0.negative_cache_ttl", "300"),
				),
			},
		}),
	})
...
// TestAccResourceNsxtPolicyDnsZone_withPartialSoa sets only primary_nameserver
// and relies on withIdempotencyChecks' PlanOnly step to catch a permadiff if
// the other soa sub-fields aren't Optional+Computed.
func TestAccResourceNsxtPolicyDnsZone_withPartialSoa(t *testing.T) {
	testResourceName := "nsxt_policy_dns_zone.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Providers: testAccProviders,
		CheckDestroy: func(state *terraform.State) error {
			return testAccNsxtPolicyDnsZoneCheckDestroy(state, displayName)
		},
		Steps: withIdempotencyChecks([]resource.TestStep{
			{
				Config: testAccNsxtPolicyDnsZoneWithPartialSoaTemplate(displayName),
				Check: resource.ComposeTestCheckFunc(
					testAccNsxtPolicyDnsZoneExists(testResourceName),
					resource.TestCheckResourceAttr(testResourceName, "soa.#", "1"),
					resource.TestCheckResourceAttr(testResourceName, "soa.0.primary_nameserver", "ns1.partial.example.com."),
				),
			},
		}),
	})
...
func testAccNsxtPolicyDnsZoneTemplate(createFlow bool) string {
	...
	return testAccNsxtDnsTransitGatewayTemplate(attrMap["display_name"]) + testAccNsxtDnsListenerIPTemplate(attrMap["display_name"]) + fmt.Sprintf(`
resource "nsxt_policy_dns_service" "parent" {
  %s
  display_name           = "%s-svc"
  allocated_listener_ips = [nsxt_policy_project_ip_address_allocation.dns_listener.path]
...
func testAccNsxtPolicyDnsZoneWithSoaTemplate() string {
	return testAccNsxtDnsTransitGatewayTemplate(accTestPolicyDnsZoneCreateAttributes["display_name"]) + testAccNsxtDnsListenerIPTemplate(accTestPolicyDnsZoneCreateAttributes["display_name"]) + fmt.Sprintf(`
resource "nsxt_policy_dns_service" "parent" {
  %s
  display_name           = "%s-svc"
  allocated_listener_ips = [nsxt_policy_project_ip_address_allocation.dns_listener.path]
...
func testAccNsxtPolicyDnsZoneWithPartialSoaTemplate(displayName string) string {
	return testAccNsxtDnsTransitGatewayTemplate(displayName) + testAccNsxtDnsListenerIPTemplate(displayName) + fmt.Sprintf(`
resource "nsxt_policy_dns_service" "parent" {
  %s
  display_name           = "%s-svc"
  allocated_listener_ips = [nsxt_policy_project_ip_address_allocation.dns_listener.path]
...
resource "nsxt_policy_dns_zone" "test" {
  parent_path     = nsxt_policy_dns_service.parent.path
  display_name    = "%s"
  dns_domain_name = "partial-soa.example.com"
  ttl             = 600

  soa {
    primary_nameserver = "ns1.partial.example.com."
  }
}`,
		testAccNsxtMultitenancyContext(false),
		displayName,
