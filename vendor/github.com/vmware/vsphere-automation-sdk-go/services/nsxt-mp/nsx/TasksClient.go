...
type TasksClient interface {
	...
	Get(taskIdParam string) (nsxModel.TaskProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, requestUriParam *string, sortAscendingParam *bool, sortByParam *string, statusParam *string, userParam *string) (nsxModel.TaskListResult, error)
}
...
func NewTasksClient(connector vapiProtocolClient_.Connector) *tasksClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.tasks")
...
func (tIface *tasksClient) Get(taskIdParam string) (nsxModel.TaskProperties, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.TaskProperties
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx.tasks", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.TaskProperties
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.TaskProperties), nil
	} else {
...
func (tIface *tasksClient) List(cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, requestUriParam *string, sortAscendingParam *bool, sortByParam *string, statusParam *string, userParam *string) (nsxModel.TaskListResult, error) {
	typeConverter := tIface.connector.TypeConverter()
	executionContext := tIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsxModel.TaskListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx.tasks", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.TaskListResult
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.TaskListResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
