...
/*
 * Client stubs for service: Stats
 * Functions that implement the generated StatsClient interface
 */
...
type DefaultStatsClient struct {
...
func NewDefaultStatsClient(connector client.Connector) *DefaultStatsClient {
	interfaceName := "com.vmware.nsx_policy.infra.settings.firewall.security.intrusion_services.stats"
	...
	sIface := DefaultStatsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	sIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	sIface.methodNameToDefMap["reset"] = sIface.resetMethodDefinition()
	return &sIface
}

func (sIface *DefaultStatsClient) Reset(categoryParam *string, enforcementPointPathParam *string) error {
...
func (sIface *DefaultStatsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := sIface.connector.GetApiProvider().Invoke(sIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (sIface *DefaultStatsClient) resetMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(sIface.interfaceName)
	typeConverter := sIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(statsResetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(statsResetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStatsClient.reset method's errors.NotFound error - %s",
