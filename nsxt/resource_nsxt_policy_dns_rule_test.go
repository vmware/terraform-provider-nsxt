...
// TestAccResourceNsxtPolicyDnsRule_domainPatternsRequired verifies that
// domain_patterns is enforced as required when action_type is
// UPDATE_MEMBERSHIP (i.e. not FORWARD+shared_zone_path).
func TestAccResourceNsxtPolicyDnsRule_domainPatternsRequired(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      testAccNsxtPolicyDnsRuleMissingDomainPatternsTemplate(displayName),
				ExpectError: regexp.MustCompile("domain_patterns is required"),
			},
		},
	})
}
...
func testAccNsxtPolicyDnsRuleMissingDomainPatternsTemplate(displayName string) string {
