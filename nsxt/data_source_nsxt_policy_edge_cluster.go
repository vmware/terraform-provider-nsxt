...
func dataSourceNsxtPolicyEdgeClusterRead(d *schema.ResourceData, m interface{}) error {
	...
	if isPolicyGlobalManager(m) {
		...
		_, err := policyDataSourceResourceReadWithValidation(d, getPolicyConnector(m), commonSessionContext, "PolicyEdgeCluster", query, false)
		if err != nil {
			return err
		}
		return nil
	...
	_, err := policyDataSourceResourceRead(d, connector, commonSessionContext, "PolicyEdgeCluster", nil)
	if err != nil {
		return err
	}
	return nil
