...
// findBareMetalServerByExternalID finds a BMS by external ID using inventory listing and filtering
func findBareMetalServerByExternalID(connector client.Connector, ctx utl.SessionContext, externalID string) (model.BareMetalServer, error) {
	...
	resultValues, err := listInventoryResourcesByType(connector, ctx, "BareMetalServer", nil)
	if err != nil {
		return found, err
	}

	servers, err := convertSearchResultToBareMetalServerList(resultValues)
	if err != nil {
		return found, err
	}
	...
	for _, server := range servers {
		if server.ExternalId != nil && *server.ExternalId == externalID {
			return server, nil
		}
	}
