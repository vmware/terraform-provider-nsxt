...
func TestUnitNsxt_dataSourceNsxtPolicyHostTransportNodeProfileRead(t *testing.T) {
	dsID := "host-tn-profile-1"

	t.Run("success", func(t *testing.T) {
		sv := policyResourceToStructValue(t, gmModel.PolicyResource{
			Id: str(dsID), DisplayName: str("name"), Path: str("/infra/x"), ResourceType: str("PolicyHostTransportNodeProfile"),
		})
		...
		ds := dataSourceNsxtPolicyHostTransportNodeProfile()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{"id": dsID})

		err := dataSourceNsxtPolicyHostTransportNodeProfileRead(d, newGoMockProviderClient())
