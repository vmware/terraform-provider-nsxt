...
type SpanClient interface {
	...
	Get(intentPathParam string, sitePathParam *string) (nsx_global_policyModel.Span, error)
}
...
func NewSpanClient(connector vapiProtocolClient_.Connector) *spanClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.span")
...
func (sIface *spanClient) Get(intentPathParam string, sitePathParam *string) (nsx_global_policyModel.Span, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.Span
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.span", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.Span
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.Span), nil
	} else {
