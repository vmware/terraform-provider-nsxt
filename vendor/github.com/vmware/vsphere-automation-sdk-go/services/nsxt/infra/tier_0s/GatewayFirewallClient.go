...
type GatewayFirewallClient interface {
	...
	List(tier0IdParam string) (nsx_policyModel.GatewayPolicyListResult, error)
}
...
func NewGatewayFirewallClient(connector vapiProtocolClient_.Connector) *gatewayFirewallClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_0s.gateway_firewall")
...
func (gIface *gatewayFirewallClient) List(tier0IdParam string) (nsx_policyModel.GatewayPolicyListResult, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_0s.gateway_firewall", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.GatewayPolicyListResult
