...
type SecurityGlobalConfigClient interface {
	...
	// @return com.vmware.nsx_global_policy.model.InfraSecurityConfig
	//
	...
	Get() (nsx_global_policyModel.InfraSecurityConfig, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(infraSecurityConfigParam nsx_global_policyModel.InfraSecurityConfig) (nsx_global_policyModel.InfraSecurityConfig, error)
}
...
func NewSecurityGlobalConfigClient(connector vapiProtocolClient_.Connector) *securityGlobalConfigClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_global_policy.global_infra.security_global_config")
...
func (sIface *securityGlobalConfigClient) Get() (nsx_global_policyModel.InfraSecurityConfig, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_global_policyModel.InfraSecurityConfig
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.security_global_config", "get", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.InfraSecurityConfig
	if methodResult.IsSuccess() {
		...
		return output.(nsx_global_policyModel.InfraSecurityConfig), nil
	} else {
...
func (sIface *securityGlobalConfigClient) Update(infraSecurityConfigParam nsx_global_policyModel.InfraSecurityConfig) (nsx_global_policyModel.InfraSecurityConfig, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.global_infra.security_global_config", "update", inputDataValue, executionContext)
	var emptyOutput nsx_global_policyModel.InfraSecurityConfig
	if methodResult.IsSuccess() {
