...
func dataSourceNsxtPolicyBareMetalServerInterfaceRead(d *schema.ResourceData, m interface{}) error {
	...
	if externalID != "" {
		...
		// Find by display name (exact match returning exactly one result)
...
// findBareMetalServerInterfaceByExternalID finds a BMS interface by external ID using inventory listing and filtering
func findBareMetalServerInterfaceByExternalID(connector client.Connector, ctx utl.SessionContext, externalID string) (model.BareMetalServerInterface, error) {
	...
	// Use inventory listing (same as bulk discovery) and filter in Go code
	resultValues, err := listInventoryResourcesByType(connector, ctx, "BareMetalServerInterface", nil)
	if err != nil {
		return found, err
	}

	interfaces, err := convertSearchResultToBareMetalServerInterfaceList(resultValues)
	if err != nil {
		return found, err
	}
	...
	for _, intf := range interfaces {
		if intf.ExternalId != nil && *intf.ExternalId == externalID {
			return intf, nil
		}
	}
