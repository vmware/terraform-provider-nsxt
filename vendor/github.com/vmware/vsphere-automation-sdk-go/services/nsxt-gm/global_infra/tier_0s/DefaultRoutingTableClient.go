...
/*
 * Client stubs for service: RoutingTable
 * Functions that implement the generated RoutingTableClient interface
 */
...
type DefaultRoutingTableClient struct {
...
func NewDefaultRoutingTableClient(connector client.Connector) *DefaultRoutingTableClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.tier_0s.routing_table"
	...
	rIface := DefaultRoutingTableClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (rIface *DefaultRoutingTableClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := rIface.connector.GetApiProvider().Invoke(rIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (rIface *DefaultRoutingTableClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(rIface.interfaceName)
	typeConverter := rIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(routingTableListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(routingTableListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultRoutingTableClient.list method's errors.NotFound error - %s",
