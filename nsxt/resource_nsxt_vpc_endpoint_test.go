...
func TestAccResourceNsxtVpcEndpoint_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy: func(state *terraform.State) error {
			return testAccNsxtVpcEndpointCheckDestroy(state, updateName)
		},
		Steps: []resource.TestStep{
			{
				Config: testAccNsxtVpcEndpointTemplate(true, createName, depsName),
				Check: resource.ComposeTestCheckFunc(
					testAccNsxtVpcEndpointExists(createName, testResourceName),
			...
			{
				Config: testAccNsxtVpcEndpointTemplate(false, updateName, depsName),
				Check: resource.ComposeTestCheckFunc(
					testAccNsxtVpcEndpointExists(updateName, testResourceName),
			...
			{
				Config: testAccNsxtVpcEndpointMinimalistic(updateName, depsName),
				Check: resource.ComposeTestCheckFunc(
					testAccNsxtVpcEndpointExists(updateName, testResourceName),
...
func TestAccResourceNsxtVpcEndpoint_importBasic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNsxtVpcEndpointMinimalistic(name, depsName),
			},
			{
...
func testAccNsxtVpcEndpointSvcVpcPrerequisites(name string) string {
...
resource "nsxt_vpc_subnet" "svc_dep" {
  context {
    project_id = "%s"
    vpc_id     = nsxt_vpc.svc_dep.nsx_id
  }
  display_name = "%s-svc-subnet"
...
resource "nsxt_vpc_service_endpoint" "dep" {
  ...
  depends_on          = [nsxt_vpc_attachment.svc_dep, nsxt_vpc_subnet.svc_dep]
}
`, projectCtx, projectCtx, name, projectCtx, name, projectCtx, name, shortID, name, projectID, name, projectID, name)
}
...
func testAccNsxtVpcEndpointDeps(depsName string) string {
	return testAccNsxtVpcEndpointSvcVpcPrerequisites(depsName) + fmt.Sprintf(`
resource "nsxt_vpc_ip_address_allocation" "dep" {
  %s
  display_name    = "%s-alloc"
  allocation_size = 1
}
`, testAccNsxtPolicyMultitenancyContext(), depsName)
}

func testAccNsxtVpcEndpointTemplate(createFlow bool, displayName, depsName string) string {
	...
	return testAccNsxtVpcEndpointDeps(depsName) + fmt.Sprintf(`
...
func testAccNsxtVpcEndpointMinimalistic(displayName, depsName string) string {
	return testAccNsxtVpcEndpointDeps(depsName) + fmt.Sprintf(`
resource "nsxt_vpc_endpoint" "test" {
  %s
  display_name         = "%s"
  vpc_service_endpoint = nsxt_vpc_service_endpoint.dep.path
  ip_allocation_path   = nsxt_vpc_ip_address_allocation.dep.path
}`, testAccNsxtPolicyMultitenancyContext(), displayName)
}
