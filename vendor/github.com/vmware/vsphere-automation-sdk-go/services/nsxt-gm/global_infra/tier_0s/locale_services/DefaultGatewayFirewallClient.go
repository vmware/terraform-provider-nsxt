...
/*
 * Client stubs for service: GatewayFirewall
 * Functions that implement the generated GatewayFirewallClient interface
 */
...
type DefaultGatewayFirewallClient struct {
...
func NewDefaultGatewayFirewallClient(connector client.Connector) *DefaultGatewayFirewallClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.tier_0s.locale_services.gateway_firewall"
	...
	gIface := DefaultGatewayFirewallClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (gIface *DefaultGatewayFirewallClient) List(tier0IdParam string, localeServicesIdParam string) (model.GatewayPolicyListResult, error) {
...
func (gIface *DefaultGatewayFirewallClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGatewayFirewallClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(gatewayFirewallListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(gatewayFirewallListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGatewayFirewallClient.list method's errors.NotFound error - %s",
