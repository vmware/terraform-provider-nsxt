...
/*
 * Client stubs for service: Query
 * Functions that implement the generated QueryClient interface
 */
...
func NewDefaultQueryClient(connector client.Connector) *DefaultQueryClient {
	interfaceName := "com.vmware.nsx_global_policy.search.query"
	...
	qIface := DefaultQueryClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (qIface *DefaultQueryClient) List(queryParam string, cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.SearchResponse, error) {
...
func (qIface *DefaultQueryClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := qIface.connector.GetApiProvider().Invoke(qIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (qIface *DefaultQueryClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(qIface.interfaceName)
	typeConverter := qIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(queryListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(queryListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultQueryClient.list method's errors.NotFound error - %s",
