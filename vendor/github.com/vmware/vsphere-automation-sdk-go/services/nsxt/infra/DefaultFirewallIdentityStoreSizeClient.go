...
/*
 * Client stubs for service: FirewallIdentityStoreSize
 * Functions that implement the generated FirewallIdentityStoreSizeClient interface
 */
...
type DefaultFirewallIdentityStoreSizeClient struct {
...
func NewDefaultFirewallIdentityStoreSizeClient(connector client.Connector) *DefaultFirewallIdentityStoreSizeClient {
	interfaceName := "com.vmware.nsx_policy.infra.firewall_identity_store_size"
	...
	fIface := DefaultFirewallIdentityStoreSizeClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (fIface *DefaultFirewallIdentityStoreSizeClient) Create(directoryDomainParam *data.StructValue, enforcementPointPathParam *string) (model.DirectoryDomainSize, error) {
...
func (fIface *DefaultFirewallIdentityStoreSizeClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := fIface.connector.GetApiProvider().Invoke(fIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (fIface *DefaultFirewallIdentityStoreSizeClient) createMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(fIface.interfaceName)
	typeConverter := fIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(firewallIdentityStoreSizeCreateInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(firewallIdentityStoreSizeCreateOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFirewallIdentityStoreSizeClient.create method's errors.NotFound error - %s",
