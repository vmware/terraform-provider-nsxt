...
func dataSourceNsxtPolicyLBClientSslProfileRead(d *schema.ResourceData, m interface{}) error {
	...
	if objID != "" {
		...
		if isNotFoundError(err) {
			return fmt.Errorf("LBClientSslProfile with ID %s was not found", objID)
		}

		if err != nil {
			return fmt.Errorf("Error while reading LBClientSslProfile %s: %v", objID, err)
		}
		obj = objGet
	} else if objName == "" {
		...
		if err != nil {
			return fmt.Errorf("Error while reading LBClientSslProfiles: %v", err)
		}
