...
func TestNSXLogicalRouterLinkPortOnTier1Basic(t *testing.T) {
...
func testAccNSXLogicalRouterLinkPortOnTier1Exists(displayName string, resourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		...
		if responseCode.StatusCode != http.StatusOK {
			return fmt.Errorf("Error while checking verifying logical router link port on Tier1 existence. HTTP returned %d", resourceID, responseCode)
		}
