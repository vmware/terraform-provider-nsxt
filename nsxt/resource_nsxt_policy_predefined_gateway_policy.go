	...
	if d.HasChange("rule") {
		...
		for _, existingDefaultRule := range predefinedPolicy.Rules {
			if existingDefaultRule.IsDefault != nil && *existingDefaultRule.IsDefault {
				...
				if len(existingDefaultRule.Scope) == 1 {
					existingDefaultScopes[existingDefaultRule.Scope[0]] = true
				}
			}
		}

		// Every configured default_rule must correspond to a default rule NSX
		// already has for this policy. There is no way for this resource to
		// create a new one, so a scope with no matching existing default rule
		// would be silently dropped causing a permadiff.
		for _, obj := range defaultRules {
			defaultRule := obj.(map[string]interface{})
			scope := defaultRule["scope"].(string)
