...
type GolangToRestVisitor struct {
	inValue  interface{}
	outValue data.DataValue
}

func NewGolangToRestDataValueVisitor(inValue interface{}) *GolangToRestVisitor {
	return &GolangToRestVisitor{inValue: inValue}
}

func (g *GolangToRestVisitor) OutputValue() data.DataValue {
	return g.outValue
}

func (g *GolangToRestVisitor) visit(bindingType BindingType) []error {
...
func (g *GolangToRestVisitor) visitReferenceType(bindingType ReferenceType) []error {
	return g.visitStructType(bindingType.Resolve().(StructType))
}

func (g *GolangToRestVisitor) visitVoidType() {
	g.outValue = data.NewVoidValue()
}

func (g *GolangToRestVisitor) visitIntegerType() []error {
...
func (g *GolangToRestVisitor) visitDoubleType() []error {
...
func (g *GolangToRestVisitor) visitStringType() []error {
...
func (g *GolangToRestVisitor) visitIdType() []error {
	return g.visitStringType()
}

func (g *GolangToRestVisitor) visitBooleanType() []error {
...
func (g *GolangToRestVisitor) visitSecretType() []error {
	if stringVal, ok := g.inValue.(string); ok {
...
func (g *GolangToRestVisitor) visitOpaqueType() []error {
...
func (g *GolangToRestVisitor) visitBlobType() []error {
	inValue := g.inValue
...
func (g *GolangToRestVisitor) visitURIType() []error {
...
func (g *GolangToRestVisitor) visitEnumType() []error {
	g.outValue = data.NewStringValue(reflect.ValueOf(g.inValue).String())
	return nil
}

func (g *GolangToRestVisitor) visitStructType(structType StructType) []error {
...
func (g *GolangToRestVisitor) visitErrorType(errorType ErrorType) []error {
	var inValue = g.inValue
	var e reflect.Value
	if isPointer(inValue) {
		e = reflect.ValueOf(inValue).Elem()
...
func (g *GolangToRestVisitor) visitDynamicStructType(structType DynamicStructType) []error {
	...
	return g.unexpectedValueError("StructValue")
}

func (g *GolangToRestVisitor) visitAnyErrorType(i AnyErrorType) []error {
...
func (g *GolangToRestVisitor) visitOptionalType(typ OptionalType) []error {
	if g.inValue == nil {
...
func (g *GolangToRestVisitor) visitListType(listType ListType) []error {
	var inValue = g.inValue
...
func (g *GolangToRestVisitor) visitMapType(mapType MapType) []error {
	inValue := g.inValue
	if g.inValue == nil || reflect.ValueOf(g.inValue).IsNil() {
		g.outValue = nil
		return []error{l10n.NewRuntimeErrorNoParam("vapi.bindings.typeconverter.value.nil")}
	}
	result := data.NewStructValue(lib.MAP_STRUCT, nil)
	...
	for _, key := range keys {
		...
		if keyStringValue, ok := keyValue.(*data.StringValue); ok {
			result.SetField(keyStringValue.Value(), g.outValue)
		} else if keyIntegerValue, ok := keyValue.(*data.IntegerValue); ok {
			result.SetField(strconv.FormatInt(keyIntegerValue.Value(), 10), g.outValue)
		} else {
			log.Errorf("Expected one of StringType, IntegerType, IdType, EnumType or UriType but found %s", reflect.TypeOf(mapType.KeyType))
			err := l10n.NewRuntimeErrorNoParam("vapi.bindings.typeconverter.dict.key.invalid")
			return []error{err}
		}
...
func (g *GolangToRestVisitor) visitSetType(setType SetType) []error {
	var inValue = g.inValue
...
func (g *GolangToRestVisitor) unexpectedValueError(expectedType string) []error {
