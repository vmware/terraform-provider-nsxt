...
func TestMockDataSourceNsxtPolicyIntrusionServiceGatewayPolicyRuleRead(t *testing.T) {
	...
	t.Run("by display_name via list", func(t *testing.T) {
		rc := int64(1)
		mockSDK.EXPECT().List(idsGwRuleDSDomain, idsGwRuleDSPolicyID, nil, nil, &boolFalse, nil, nil, nil, nil).Return(nsxModel.IdsRuleListResult{
	...
	t.Run("list error", func(t *testing.T) {
		mockSDK.EXPECT().List(idsGwRuleDSDomain, idsGwRuleDSPolicyID, nil, nil, &boolFalse, nil, nil, nil, nil).Return(nsxModel.IdsRuleListResult{}, vapiErrors.InternalServerError{})
