...
func TestUnitNsxt_dataSourceNsxtPolicyMacDiscoveryProfileRead(t *testing.T) {
	dsID := "mac-discovery-profile-1"

	t.Run("success", func(t *testing.T) {
		sv := policyResourceToStructValue(t, gmModel.PolicyResource{
			Id: str(dsID), DisplayName: str("name"), Path: str("/infra/x"), ResourceType: str("MacDiscoveryProfile"),
		})
		...
		ds := dataSourceNsxtPolicyMacDiscoveryProfile()
		d := schema.TestResourceDataRaw(t, ds.Schema, map[string]interface{}{"id": dsID})

		err := dataSourceNsxtPolicyMacDiscoveryProfileRead(d, newGoMockProviderClient())
