...
A `DnsService` is deployed on VNS clusters and provides authoritative DNS resolution for VPCs. Listener IPs must reference valid `IpAddressAllocation` objects within the same project.
