...
~> **NOTE:** This resource is **not supported on NSX Global Manager**: unlike Gateway Policies, Global Manager does not create category-level Default Security Policies, so there is no default rule for this resource to modify. Attempting to use this resource on a Global Manager provider configuration (`global_manager = true`) will fail.

~> **NOTE:** `default_rule` can only be used against the Default Security Policy for a given Category, as those are the only sections with a default rile.
