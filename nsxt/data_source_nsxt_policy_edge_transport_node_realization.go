		...
		Refresh: func() (interface{}, string, error) {
			...
			if state.EdgeTnState == nil {
				...
				if *state.EdgeTnState.ConsolidatedStatus == model.EdgeTnState_CONSOLIDATED_STATUS_ERROR {
					return state, *state.EdgeTnState.ConsolidatedStatus, fmt.Errorf("transport node %s failed to realize with state %v: %v", id, state, state.EdgeTnState.FailureMessage)
				}

				d.Set("state", state.EdgeTnState.State)
