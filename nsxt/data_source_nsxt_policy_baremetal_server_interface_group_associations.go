...
import (
	...
	cliBareMetalServerInterfaceGroupAssociationsClient "github.com/vmware/terraform-provider-nsxt/api/infra"
	utl "github.com/vmware/terraform-provider-nsxt/api/utl"
)

func dataSourceNsxtPolicyBareMetalServerInterfaceGroupAssociations() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceNsxtPolicyBareMetalServerInterfaceGroupAssociationsRead,
		...
		Schema: map[string]*schema.Schema{
			...
			"groups": {
				...
				Description: "List of groups this bare metal server interface is a member of",
				Computed:    true,
...
func dataSourceNsxtPolicyBareMetalServerInterfaceGroupAssociationsRead(d *schema.ResourceData, m interface{}) error {
	...
	client := cliBareMetalServerInterfaceGroupAssociationsClient.NewBareMetalServerInterfaceGroupAssociationsClient(sessionContext, connector)

	associations, err := client.List(externalID, nil, enforcementPointPathPtr, nil, nil, nil, nil, nil)
	if err != nil {
		return fmt.Errorf("Failed to read group associations for bare metal server interface %s: %v. Verify the interface exists and is accessible via Policy API", externalID, err)
	}
