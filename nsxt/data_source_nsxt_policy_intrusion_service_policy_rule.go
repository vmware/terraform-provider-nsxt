...
func dataSourceNsxtPolicyIntrusionServicePolicyRule() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceNsxtPolicyIntrusionServicePolicyRuleRead,
		Schema: map[string]*schema.Schema{
			...
			"oversubscription": {
				Type:         schema.TypeString,
				Description:  "Indicates how rule performs when oversubscribed",
				Optional:     true,
				Computed:     true,
...
func listIntrusionServicePolicyRules(context utl.SessionContext, policyPath string, m interface{}) ([]model.IdsRule, error) {
	connector := getPolicyConnector(m)

	domain := getDomainFromResourcePath(policyPath)
	policyID := getPolicyIDFromPath(policyPath)

	client := cliIntrusionServicePolicyRulesClient(context, connector)
	if client == nil {
...
func dataSourceNsxtPolicyIntrusionServicePolicyRuleRead(d *schema.ResourceData, m interface{}) error {
	...
	if objID != "" {
		...
		if err != nil {
			if isNotFoundError(err) {
				return fmt.Errorf("Intrusion Service Policy Rule with ID %s was not found in policy %s", objID, policyID)
			}
			return fmt.Errorf("Error while reading Intrusion Service Policy Rule %s: %v", objID, err)
		}
		d.SetId(*obj.Id)
		setIntrusionServicePolicyRuleDataSourceSchema(d, obj)
		return nil
	}

	if objName == "" {
		return fmt.Errorf("Intrusion Service Policy Rule id or display_name must be specified")
	}

	rules, err := listIntrusionServicePolicyRules(context, policyPath, m)
	if err != nil {
		return fmt.Errorf("Error while reading Intrusion Service Policy Rules: %v", err)
	}
	...
	if len(perfectMatch) > 0 {
		if len(perfectMatch) > 1 {
			return fmt.Errorf("Found multiple Intrusion Service Policy Rules with name '%s'", objName)
		}
		obj = perfectMatch[0]
	} else if len(prefixMatch) > 0 {
		if len(prefixMatch) > 1 {
			return fmt.Errorf("Found multiple Intrusion Service Policy Rules with name starting with '%s'", objName)
		}
		obj = prefixMatch[0]
	} else {
		return fmt.Errorf("Intrusion Service Policy Rule with name '%s' was not found in policy %s", objName, policyID)
	}
...
func setIntrusionServicePolicyRuleDataSourceSchema(d *schema.ResourceData, obj model.IdsRule) {
